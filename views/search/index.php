...
?>
<div class="container">
    <p>There are <strong><?=$count?></strong> Airbnb in <strong><?=$search_key?></strong>
        <a href="<?=HOST;?>/index.php?r=analysis/analysis&search_key=<?=$search_key?>">See Market report</a></p>
...
<div class="row">
    ...
    <div class="col-lg-4">
        <div id="mainmap" style="width: 400px;height: 600px"></div>
        <script>
            jQuery(function($) {
                // Asynchronously Load the map API
                var script = document.createElement('script');
                script.src = "//maps.googleapis.com/maps/api/js?key=AIzaSyDtHYiCTNV2ung9PUph6cOHktt25qGtq_U&language=en&callback=initialize";
            ...
            function initialize() {
                ...
                var markers = [
                    <?php foreach ($listings as $listing): ?>
                    <?php
                    $listing->NAME = str_replace("'","\"",$listing->NAME);
                    $listing->SUMMARY = str_replace("'","\"",$listing->SUMMARY);
                    ?>
                    ['<?=$listing->NAME?>', <?=$listing->LATITUDE?>,<?=$listing->LONGITUDE?>],
                    <?php endforeach; ?>
                ];
                ...
                var infoWindowContent = [
                    <?php foreach ($listings as $listing): ?>
                    ['<div class="info_content" style="width:300px;">' +
                    '<a onclick="detail(<?=$listing->LISTING_ID?>)"><h3><?=$listing->NAME?></h3></a>' +
                    '<p><?=$listing->SUMMARY?></p>' +        '</div>'],
                    <?php endforeach; ?>
                ];
